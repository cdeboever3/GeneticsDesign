GeneticPower.Quantitative.Factor <- function(N=1000,delta=1,freq=0.15,minh=c('additive','dominant','recessive'),
                                      sigma=1,OtherParms=0,alpha=0.05, numtests=1, moi=NULL, rsquared=NULL) 
{
...
# rsquared = fraction of total sum-of-squares explained by fit. OVER-RIDES delta AND sigma.
    ...
    lambda <- ifelse(is.null(rsquared) ,(N1*(mu[1]-mu.bar)^2+N2*(mu[2]-mu.bar)^2+N3*(mu[3]-mu.bar)^2)/(sigma^2),(N-3)*rsquared/(1-rsquared));

    return(pf(qf(1-alphy,df1=2,df2=(N-3-OtherParms)),ncp=lambda,df1=2,df2=(N-3-OtherParms),lower.tail=F)); # this is power
} # end of GeneticPower.Quantitative.
